...
def plot(plots, xc, Vc, zc, DEL, eats, nKGB, KGB, DATA, zpoints, fpoints, sig80):
    ...
    if any(string == 'w' for string in plots):
        ...
        #plt.plot(DATA[2], DATA[0], color='g', label='Toy ``Slinky" Model $B = 20$', linewidth=0.5)
        #plt.text(5.4, -0.6, '$f(\phi) = \sin(\eta \,(\phi - \phi_{c}))$', fontsize=11)
        plt.xlim(left=0, right=2)
        plt.ylim(top=-0.8)
        plt.xlabel('$z$', fontsize=24)
        plt.ylabel('$ w $', fontsize=24)
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[4+(j+1)*DEL], color=colors[2+2*j], zorder=j, linestyle=lines[1+j],\
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        if KGB == 'y' :
            for i in range(len(nKGB)):
                plt.plot(DATA[2], DATA[20+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
...
#                         alpha=0.25, facecolor='b', zorder=len(eats)+2)
    ...
    if any(string == 'H' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[5+(j+1)*DEL], color=colors[2*j+2], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        if KGB == 'y' :
            for i in range(len(nKGB)):
                plt.plot(DATA[2], DATA[21+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[15+(j+1)*DEL], color=colors[2*j+2], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        if KGB == 'y' :
            for i in range(len(nKGB)):
                plt.plot(DATA[2], DATA[23+(i+1)*DEL], color=colors[len(eats)+2+2*i], \
                         label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), linewidth=0.85, linestyle=lines[len(eats)+1+i])
        plt.plot(DATA[2], DATA[10], label='$\Lambda$CDM', linewidth=0.85, color='k')
        plt.xlim(left=0, right=DATA[2,0])
        plt.ylim(bottom=0)
        plt.xlabel('$z$')
        ...
        format_label_string_with_exponent(ax15, axis='both')
    ...
    if any(string == 'dL' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[13+(j+1)*DEL], color=colors[2*j+2],linestyle=lines[1+j], \
                    label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[12+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.plot(DATA[2], DATA[10], color='k', label='$\Lambda$CDM', linewidth=0.85)
        plt.xlim(left=0, right=0.5)
        plt.ylim(bottom=0, top=0.5*10**42.)
        plt.xlabel('$z$')
        plt.ylabel('$d_L$\ [GeV]')
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[14+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j],\
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        if KGB == 'y' :
            for i in range(len(nKGB)):
                plt.plot(DATA[2], DATA[24+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                         linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])        
...
#                   label='Current SN constraint on \n $\Lambda$CDM deviation')
        ...
        format_label_string_with_exponent(ax5, axis='both')
    ...
    if any(string == 'g' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[9+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j],\
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[29+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]),\
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.plot(DATA[2], DATA[8], color='k', label='$\Lambda$CDM', linewidth=0.85)
        ...
        format_label_string_with_exponent(ax6, axis='both')
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[38+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j],\
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[39+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.plot(DATA[2], DATA[13], color='k', label='$\Lambda$CDM', linewidth=0.85)
        ...
        format_label_string_with_exponent(ax21, axis='both')
        ...
        plt.plot(DATA[2], DATA[12], color='c', label='$\Lambda$CDM Numerical', linewidth=0.85)
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[8+(j+1)*DEL], color=colors[2*j+2],\
                     label='\emph{{STSQ}} Numerical $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[32+(i+1)*DEL], label='\emph{{KGB}} Numerical $n={0}$'.format(nKGB[i]),\
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
            nall = [1,2,3,4,5]
            if any(x == nKGB[i] for iii, x in enumerate(nall)):
                plt.plot(DATA[2], DATA[33+(i+1)*DEL], label='\emph{{KGB}} Analytic $n={0}$'.format(nKGB[i]), \
                         linewidth=0.85, color=colors[len(eats)+2+2*i])
...
#        plt.hlines(0.51545454545, 0, 3, linewidth=0.85, color='g')
        ...
        format_label_string_with_exponent(ax7, axis='both')
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[10+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[31+(i+1)*DEL],label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
...
#        plt.plot(DATA[2], 2*DATA[0], label='$WFIRST$ Aggregate Precision $2\sigma$', linewidth=0.85, color='g')
        ...
        plt.plot(DATA[2], DATA[11], color='k', label= '$\Lambda$CDM', linewidth=0.85)
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[35+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[34+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.xlim(left=0, right=DATA[2,0])
        plt.xlabel('$z$')
        plt.ylabel('$\Delta g$')
        ...
        format_label_string_with_exponent(ax19, axis='both')
...
#        format_label_string_with_exponent(ax14, axis='both')
...
#        plt.plot(DATA[2], DATA[6], label='$\Omega_{DE}$ $\Lambda$CDM', linewidth=0.85, color='k')
    ...
    if any(string == 'phi' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[1], DATA[1+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j], \
                    label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[1], DATA[26+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.xlim(left=DATA[1,0], right=1)
        plt.ylim(bottom=2.26*10**19, top=2.275*10**19)
        plt.xlabel('$a$')
        plt.ylabel('$\phi$ [$GeV$]')
        plt.legend(frameon=False, fontsize=10)
    ...
    if any(string == 'V' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[1], DATA[3+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        plt.xlim(left=DATA[1,0], right=1)
...
#        
    if any(string == 'q' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[16+(j+1)*DEL], color=colors[2*j+2], \
                     label='\emph{{STSQ}} $z_c={0}$ $\eta$={1}'.format(zc, eats[j]), linewidth=0.85)
        for i in range(len(nKGB)):
            plt.plot(DATA[2], DATA[25+(i+1)*DEL], label='\emph{{KGB}}  $n={0}$'.format(nKGB[i]), \
                     linewidth=0.85, color=colors[len(eats)+2+2*i], linestyle=lines[len(eats)+1+i])
        plt.xlim(left=0, right=5)
#        plt.ylim(top=0.15, bottom=-0.025)
        plt.yticks(np.arange(-0.7, 0.7, step=0.2))
        plt.xlabel('$z$')
        plt.ylabel('$q$')
        plt.legend(frameon=False, fontsize=10)
        ax16.tick_params(axis='both', direction='in')
        format_label_string_with_exponent(ax16, axis='both')
    ...
    if any(string == 'sc' for string in plots):
        ...
        for j in range(len(eats)):
            plt.plot(DATA[2], DATA[28+(j+1)*DEL], color=colors[2*j+2], linestyle=lines[1+j], \
                     label='\emph{{STSQ}}  $z_c={0}$ $\eta={1}$'.format(zc, eats[j]), linewidth=0.85)
        plt.xlim(left=0, right=DATA[2,0])
        plt.xlabel('$z$')
        plt.ylabel('$\\xi$')
        ...
        format_label_string_with_exponent(ax18, axis='both')
    ...
    if any(string == 'att' for string in plots):
        ...
        plt.xlabel('$V(\phi)$ [$GeV^{4}$]')
        plt.ylabel('$\dot{\phi}^2$ [$GeV^{4}$]')
        #plt.legend(frameon=False, fontsize='small')
        ax22.tick_params(axis='both', direction='in')
        format_label_string_with_exponent(ax22, axis='both')
        plt.savefig('attractorSTSQ.png', dpi=500, format='png')

        fig24 = plt.figure(24)
        ax24 = fig24.add_subplot(111)
        for j in range(len(nKGB)):
            plt.plot(DATA[21+(j+1)*DEL], DATA[27+(j+1)*DEL],linewidth=0.85, color=colors[2*j])
        plt.xlabel('$H^{-1}$')
        plt.ylabel('$\dot{\phi}$')
        #plt.legend(frameon=False, fontsize='small')
        ax24.tick_params(axis='both', direction='in')
        format_label_string_with_exponent(ax24, axis='both')
    ...
    if any(string == 'G' for string in plots):  
        ...
        for j in range(len(nKGB)):
            plt.plot(DATA[1], DATA[40+(j+1)*DEL],linewidth=0.85, color=colors[2*j+2],\
        ...
        format_label_string_with_exponent(ax23, axis='both')
